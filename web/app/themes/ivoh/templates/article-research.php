...
<article class="research<?= $external_link ? ' with-icon' : ''?>">
  <h2 class="article-title"><a <?= $external_link ? 'rel="noopener" target="_blank" ' : '' ?>href="<?= $research_url ?>"><?= $research_post->post_title ?><?php if ($external_link): ?><svg class="icon icon-outbound-link" aria-hidden="true" role="presentation"><use xlink:href="#icon-outbound-link"/></svg><?php endif ?></a></h2>
